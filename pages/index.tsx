...
import { PayWithBankTransfer, SelectPaymentType, PayWithCard } from "@/components";
...
const Home = () => {
    ...
    useEffect(() => {
        ...
        socketRef.current.on(`${accountDetails?.reference}-${accountDetails?.accountNumber}`, (data) => {
            setPaymentDetails(data);
        });
    }, [accountDetails?.reference, accountDetails?.accountNumber, accountDetails?.sessionKey, isMounted]);
    ...
    const generateAccountNumberFn = async () => {
        ...
        try {
            ...
            setError(err?.response?.data?.message || "Transaction could not be initiated");
        } finally {
            setLoading(false);
        }
    };
    ...
    const renderContent = () => {
        switch (status) {
            ...
            case "card": {
                return <PayWithCard setStatus={setStatus} customerDetails={customerDetails} />;
            }
            default: {
                return (
                    <SelectPaymentType
